...
def main():
    ...
    st.write(
        "Загрузите изображение объекта и фона, чтобы выполнить сегментацию и зам"
        + "ену фона.\nИзображения объекта и фона должны быть одного разрешения"
    )
        ...
        with mp_selfie_segmentation.SelfieSegmentation() as selfie_segmentation:
            try:
                ...
                output_image = np.where(
                    condition, np.array(obj_image), np.array(bg_image)
                )
